...
Inst Assembler::li(std::vector<std::wstring> args) {
  ...
  return 14 << (16 + 5 + 5) | regno << (16 + 5) | value;
}
Inst Assembler::stwu(std::vector<std::wstring> args) {
  auto regno = util::_toInt(args[0].substr(1));
  uint16_t imm = util::_toInt(args[1].substr(0, args[1].find(L'(')));
  auto srcRegno = util::_toInt(args[1].substr(
      args[1].find(L'(') + 2, args[1].find(L')') - args[1].find(L'(') - 2));
  return 37 << (16 + 5 + 5) | regno << (16 + 5) | srcRegno << (16) | imm;
}
Inst Assembler::mflr(std::vector<std::wstring> args) {
  auto regno = util::_toInt(args[0].substr(1));
  return 0x7c0802a6 | regno << 21;
}
Inst Assembler::stw(std::vector<std::wstring> args) {
  ...
  auto srcRegno = util::_toInt(args[1].substr(
      args[1].find(L'(') + 2, args[1].find(L')') - args[1].find(L'(') - 2));
  return 36 << (16 + 5 + 5) | regno << (16 + 5) | srcRegno << (16) | imm;
}
Inst Assembler::lwz(std::vector<std::wstring> args) {
  ...
  auto srcRegno = util::_toInt(args[1].substr(
      args[1].find(L'(') + 2, args[1].find(L')') - args[1].find(L'(') - 2));
  return 32 << (16 + 5 + 5) | regno << (16 + 5) | srcRegno << (16) | imm;
}
...
Inst Assembler::mtlr(std::vector<std::wstring> args) {
  auto regno = util::_toInt(args[0].substr(1));
  return 0x7c0803a6 | regno << 21;
}
Inst Assembler::mtctr(std::vector<std::wstring> args) {
  auto regno = util::_toInt(args[0].substr(1));
  return 0x7c0903a6 | regno << 21;
}
Inst Assembler::blr(std::vector<std::wstring> args) { return 0x4e800020; }
Inst Assembler::bctrl(std::vector<std::wstring> args) { return 0x4e800421; }
